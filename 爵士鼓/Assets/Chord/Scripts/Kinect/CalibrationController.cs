...
namespace ChordPlayer.Kinect
{
    ...
    public class CalibrationController : MonoBehaviour
    {
        ...
        // NOTE: original chord project had a MicrosoftKinectInputProvider reference here
        // to detect "sensor disconnected" vs "no body". Our drum project gets body data
        // via UDP, so there is no in-process SDK to query. We drop that branch and
        // simply show the step-in prompt whenever the body is not tracked. _msgDisconnected
        // is kept as a serialized field for scene compatibility but is no longer used.
        float _timer;
        bool _done;
        float _doneFlash;
        ...
        void Update()
        {
            ...
            if (!tracked) { Show(_msgStepIn); _timer = 0f; return; }
